...
;(function (win,doc) {
    var host = window.location.host === "121.15.167.251:31031" ? "http://120.197.235.102" : "https://www.cmpassport.com"
	var opts = {
		//取号地址
        getMobileUrl:{
            test01:"http://120.197.235.102/NumberAbility/h5/getMobile.htm?",
            // pro:"http://verify.cmpassport.com/NumberAbility/h5/getMobile.htm?"
            pro:"http://www.cmpassport.com/NumberAbility/h5/getMobile.htm?"
        },
        //获取token
        getToken:{
            test01:"http://120.197.235.102/NumberAbility/h5/getToken.htm",
            // pro:"https://verify.cmpassport.com/NumberAbility/h5/getToken.htm"
    ...
    win.YDRZ = {
        ...
        getSign:function(appId,version){
            opts.optparams.uuid = uuid(32,32);
        ...
        getTokenInfo:function(opt){
            ...
            var expandParams = opt.data.expandParams === '' ? "callType=6005" : "callType=6005|" + opt.data.expandParams;
            var expandParamsToken = opt.data.expandParams === '' ? "callType=6006" : "callType=6006|" + opt.data.expandParams;
            //判断参数是否正确
                var options = {
                    //业务方参数
                ...
                + "&expandParams=" + encodeURIComponent(expandParams)
                + "&msgId=" + options.msgId
                    	...
                    	request.ajax({
                    		...
                    		success:function(res){
                    			if (res.result.body.token != '') {
                    				...
                    				opt.success({code:obj.code,message:obj.message,token:obj.token,userInformation:obj.userInformation})
                    			}else{
                    				var obj = {
                    					code:res.result.header.resultCode,
                ...
                img.onerror = function(){
                    ...
                    opt.error({code:"500",message:"取号失败",msgId:options.msgId}) 
                    This.isLoaded = true;
                    doc.body.removeChild(img);
                }
        }
   ...
   function checkParms(opt){
	   	...
	   	for (p in opt.data) {
	   		if (opt.data[p] === '' || opt.data[p] === undefined){
	   			opt.error({code:"500",message:p+"为空，请检查参数是否正确"})
	   			isflag = false;
	   			return
	   		}
	   	}
