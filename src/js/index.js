...
window.onload = function () {
    ...
    // 获得实物打开
    // !function popGetRealPrize(){
    //     dialog.confirm('自定义按钮', '恭喜您，成功抢到<span class="shiwu">MM公仔</span><div class="tip"><i class="tip-icon">i</i>该奖品将通过快递方式寄送给您，请在活动当天填写收货信息，以免错失奖品！</div>', [
    //         {
    //             txt: '取消',
    //             callback: function () {
    //                 dialog.toast('你点了取消', 'none', 1000);
    ...
    function checkValue(obj){
        ...
        if(obj.username.length===0){
            dialog.toast('请输入收货人', 'none', 1000);
            flag = false;
            return flag;
        }
        if(obj.username.length>8){
            dialog.toast('最多输入8个汉字', 'none', 1000);
            flag = false;
            return flag;
        } 
        if(obj.tel.length===0){
        ...
        if(obj.adress.length===0){
            dialog.toast('请输入详细地址', 'none', 1000);
            flag = false;
            return flag;
        } 

        if(obj.adress.length>30){
            dialog.toast('最多输入30个汉字', 'none', 1000);
            flag = false;
            return flag;
        }
        return flag;
    ...
    !function () {
        ...
        $target.on('done.ydui.cityselect', function (ret) {
            $('.address-value').text(ret.provance + ' ' + ret.city + ' ' + ret.area);
        });
    }();
    ...
    var $targetObj12 = {
        slot:$('.12-time'),
        h:$('.MHandSecKill_Tag_12').find('.hour'),
        m:$('.MHandSecKill_Tag_12').find('.min'),
        s:$('.MHandSecKill_Tag_12').find('.secent')
    };
    var $targetObj19 = {
        slot:$('.19-time'),
        h:$('.MHandSecKill_Tag_19').find('.hour'),
        m:$('.MHandSecKill_Tag_19').find('.min'),
        s:$('.MHandSecKill_Tag_19').find('.secent')
    };
    // 倒计时动效
    /**
     * js倒计时
     ...
     * @param speed 速度
     * @param callback ret 倒计时结束回调函数 ret 时间对象 ；ret == 'end' 则倒计时结束
     */
    //  YDUI.util.countdown('{%h}:{%m}:{%s}', Date.parse(new Date()) + 6000, 1000, function(obj){ 
    //      if(obj!=='end'){
    //          console.log(obj); 
    //          getPrizeByLeftTime(obj,$targetObj12);
    //         
    //      }else{
    //          $targetObj12.slot.addClass('not-beigin');
    //           //popOnNoBeigin()
    //      }
    //     
    //  });
    ...
    function popOnNoBeigin(){
        dialog.notBeigin('',
            '<div class="share-on-web not-beigin"><span>上半场已结束!<span></div>',
            []);
    }
    // 秒杀活动时间轮训
    function getPrizeByLeftTime(obj,$targetObj){
        $targetObj.h.text(obj.h);
        $targetObj.m.text(obj.m);
        $targetObj.s.text(obj.s);
    }
