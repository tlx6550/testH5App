...
class LotteryCard extends Events {
    ...
    init () {
        ...
        const imgW = this.options.bgImgW
        const imgH = this.options.bgImgH
        if (cover instanceof Image) {
            //根据图片大小实际填充满
            this.ctx.drawImage(cover,0,0,imgW,imgH);
           /* this.ctx.fillStyle = this.ctx.createPattern(cover, 'no-repeat')
